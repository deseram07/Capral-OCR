...
def process_request(input_file, output_file, lang=None, psm=None, makebox=None, digit=None):
...
def iplimage_to_string(im, lang=None, psm=None, makebox=None,digit=None):
    if not OPENCV_AVAILABLE:
        print "OpenCV not Available"
        return -1
    else:
        cv.SaveImage(TEMP_IMAGE, im)
        txt = image_to_string(TEMP_IMAGE, lang, psm, makebox, digit)
        os.remove(TEMP_IMAGE)
        return txt
        
def image_to_string(file,lang=None, psm=None, makebox=None, digit=None):
    # check_path() #Check if tesseract available in the path
    process_request(file, TEMP_FILE, lang, psm, makebox, digit) #Process command
    if makebox is not None:
        ext = ".box"
    else:
        ext = ".txt"

    f = open(TEMP_FILE+ext,"r") #Open back the file
    txt = f.read()
    f.close()

    if makebox is not None:
        os.remove(TEMP_FILE+ ".box")
    else:
...
if __name__ =='__main__':
    print image_to_string("image.jpg", "eng", PSM_AUTO) #Example